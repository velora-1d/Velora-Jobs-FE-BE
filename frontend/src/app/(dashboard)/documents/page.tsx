            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="glass-panel p-6 rounded-3xl flex flex-col max-h-[600px]">
                    <h2 className="text-xl font-bold text-foreground mb-6 flex items-center gap-2">
                        <span className="w-8 h-8 rounded-full bg-blue-500/10 text-blue-500 flex items-center justify-center text-sm font-mono border border-blue-500/30">1</span>
                    ...
                    <div className="space-y-4 overflow-y-auto pr-2 flex-1">
                        ...
                        {leads.map(lead => (
                            <div
                                ...
                                className={`p-4 rounded-xl border cursor-pointer transition-all ${selectedLeadId === lead.id ? 'bg-blue-500/10 border-blue-500 text-blue-500' : 'bg-card border-border hover:bg-accent/10 text-muted-foreground'}`}
                            >
                                <div className="flex justify-between items-center">
                                    <span className="font-medium truncate max-w-[200px]">{lead.title}</span>
                                    {selectedLeadId === lead.id && <CheckSquare className="w-5 h-5 text-blue-500" />}
                                </div>
                ...
                <div className="glass-panel p-6 rounded-3xl flex flex-col items-center justify-center text-center opacity-50 relative">
                    {!selectedLeadId && <div className="absolute inset-0 bg-background/50 z-10 cursor-not-allowed" />}
                    <div className={`transition-opacity ${selectedLeadId ? 'opacity-100' : 'opacity-30'}`}>
                        <FilePlus className="w-16 h-16 text-muted-foreground mb-4 mx-auto" />
                        <h2 className="text-xl font-bold text-foreground mb-2">Generate Proposal</h2>
                        <p className="text-muted-foreground mb-8 max-w-xs mx-auto">Create a custom proposal PDF for the selected client with standard pricing.</p>

                        <button
                            onClick={generateProposal}
                            disabled={!selectedLeadId || isGenerating}
                            className="bg-emerald-600 hover:bg-emerald-500 text-white px-8 py-4 rounded-2xl font-bold text-lg flex items-center gap-3 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-emerald-500/20"
                        >
